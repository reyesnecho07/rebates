...
router.post('/fix-cust-rebate', async (req, res) => {
  try {
    ...
    // SIMPLE TEST QUERY - Let's make it as simple as possible
    const query = `
      INSERT INTO FixCustRebate (RebateCode, CardCode, CardName, QtrRebate, CreatedDate)
      VALUES (@RebateCode, @CardCode, @CardName, @QtrRebate, GETDATE())
    `;
    ...
    // Add ONLY these parameters - NO Id!
    ...
    // Get the inserted ID separately
    const idResult = await req.db.request().query(`
      SELECT SCOPE_IDENTITY() as NewId
    `);
    
    const newId = idResult.recordset[0].NewId;
    
    console.log(`✅ Fixed customer rebate saved successfully. ID: ${newId}`);
...
// Save to FixCustQuota table - NEW STRUCTURE (one row per month)
router.post('/fix-cust-quota', async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO FixCustQuota (CustRebateId, Month, TargetQty, CreatedDate)
      VALUES (@CustRebateId, @Month, @TargetQty, GETDATE());
    ...
    console.log(`✅ Fixed customer quota saved successfully. ID: ${newId}`);
...
// Bulk save to FixCustQuota table (for saving all months at once)
router.post('/fix-cust-quotas/bulk', async (req, res) => {
  try {
    const { CustRebateId, quotas } = req.body; // quotas is an array of {Month, TargetQty}
    
    console.log(`💾 Bulk saving fixed customer quotas to database: ${req.database}`);
    console.log('📤 Data received:', { CustRebateId, quotas });
    
    if (!Array.isArray(quotas) || quotas.length === 0) {
      return res.status(400).json({
        success: false,
        error: 'No quotas provided',
        database: req.database
      });
    }
    ...
    console.log(`✅ ${results.length} fixed customer quotas saved successfully.`);
...
// Save to PerCustQuota table - NEW STRUCTURE
...
// Bulk save to PerCustQuota table (for saving all months at once)
router.post('/per-cust-quotas/bulk', async (req, res) => {
  try {
    const { CustRebateId, quotas } = req.body; // quotas is an array of {Month, TargetQty}
    
    console.log(`💾 Bulk saving percentage customer quotas to database: ${req.database}`);
    console.log('📤 Data received:', { CustRebateId, quotas });
    ...
    console.log(`✅ ${results.length} percentage customer quotas saved successfully.`);
