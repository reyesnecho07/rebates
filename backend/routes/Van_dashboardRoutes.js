...
const getPoolWithFallback = async (databaseName) => {
  ...
  if (!pool && databaseName !== 'VAN') {
    console.log(`⚠️ Database ${databaseName} not available, trying VAN...`);
    const fallbackPool = getDatabasePool('VAN');
    return fallbackPool;
  }
...
router.get('/rebate/:rebateCode/details', async (req, res) => {
  ...
  try {
    ...
    const rebateInfoQuery = `
      SELECT RebateCode, RebateType, SlpName as salesEmployee, DateFrom, DateTo, IsActive, Frequency, QuotaType, Name
      FROM RebateProgram 
...
router.get('/test-item-matching/:customerCode/:rebateCode', async (req, res) => {
  try {
    ...
    const ownPool = await getPoolWithFallback('VAN');
    const sapPool = getDatabasePool('VAN');
...
router.get('/customer/:customerCode/monthly-quota', async (req, res) => {
  try {
    ...
    const host = req.headers.host || 'localhost:3009';
  ...
  try {
    ...
    try {
      ...
      const totalUnpaidRebateQuery = `
          SELECT 
            SUM(ISNULL(BaseAmount, 0)) - SUM(ISNULL(AmountReleased, 0)) AS [Total Unpaid Rebate]
          FROM PayoutHistory
