...
function Vcp_ItemRecords() {
  ...
  // Fetch items from local VCP_OWN database
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await fetch("http://192.168.100.193:3006/api/sync/local/items?db=VCP_OWN");
        ...
        setItems(itemsData);
        console.log(`Loaded ${itemsData.length} items from VCP_OWN database`);
      } catch (err) {
