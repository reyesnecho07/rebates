...
function Vcp_CustomerRecords() {
  ...
  // Fetch customers from local VCP_OWN database
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const res = await fetch("http://192.168.100.193:3006/api/sync/local/customers?db=VCP_OWN");
        ...
        console.log(`Loaded ${customersData.length} customers from VCP_OWN database`);
      } catch (err) {
          ...
          <div className={`bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-3xl border ${
            ...
            <div className={`flex items-center gap-3 mb-6 pb-4 border-b ${
              ...
              <div>
                ...
                }`}>Manage and view all customer information from VCP_OWN database</p>
              </div>
            </div>
