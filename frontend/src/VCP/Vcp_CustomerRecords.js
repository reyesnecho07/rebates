...
import { Users, Search, Filter, X, Check } from "lucide-react";
...
const componentMetadata = {
  name: 'Vcp_CustomerRecords',
  version: '2.0.0',
  description: 'Displays and manages a list of customer records.',
  routePath: '/Vcp_CustomerRecords',
};

function Vcp_CustomerRecords() {
  const { theme, updateTheme } = useTheme();
  const location  = useLocation();
  const routePath = '/Vcp_CustomerRecords';
  ...
  // ── Fetch customers (only when access is granted) ─────────────────────────
  useEffect(() => {
    if (!access.canView) return;
    const fetchCustomers = async () => {
      try {
        const res  = await fetch("http://192.168.100.193:3009/api/sync/local/customers?db=VCP_OWN");
  ...
  const uniqueGroups = useMemo(() => {
    const s = new Set();
    customers.forEach(c => {
      const g = c.GroupName || c.CardGroup || c.groupName || c.custGroup || '';
      if (g) s.add(g);
    });
  ...
  const visibleGroups = useMemo(() => {
    if (!groupSearch.trim()) return uniqueGroups;
    const t = groupSearch.toLowerCase();
    return uniqueGroups.filter(g => g.toLowerCase().includes(t));
  }, [uniqueGroups, groupSearch]);
  ...
  const filteredCustomers = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(c =>
        ...
        Object.values(c).join(' ').toLowerCase().includes(s)
      );
    }
    return result;
  }, [customers, search, selectedGroups]);
          ...
          <div className={`flex-1 flex flex-col rounded-2xl border shadow-sm w-full max-w-[1600px] mx-auto mt-4 overflow-hidden ${
            ...
            {accessLoading ? (
              ...
              <AccessDenied
                ...
                message="You don't have permission to view customer records."
              />
            ) : (
              <>
                {/* ── Page header ──────────────────────────────────────────── */}
                <div className={`flex-shrink-0 flex items-center justify-between gap-4 px-6 py-4 border-b ${
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      ...
                      <Users size={17} className={isDark ? 'text-blue-400' : 'text-blue-400'} strokeWidth={1.6} />
                    </div>
                    <div className="min-w-0">
                      <h1 className={`text-sm font-bold leading-none ${T.tp}`}>Customer Records</h1>
                      <p className={`text-[11px] mt-0.5 ${T.ts}`}>Manage and view all customer records</p>
                    </div>
                  </div>
                        ...
                        <div className={`absolute right-0 top-full mt-2 w-80 rounded-xl shadow-2xl border z-50 overflow-hidden ${
                          ...
                          <div className={`flex items-center justify-between px-4 py-2.5 border-t ${
                            ...
                            <span className={`text-[10px] ${T.tm}`}>
                              {uniqueGroups.length} group{uniqueGroups.length !== 1 ? 's' : ''} total
                            </span>
                            <button
                              onClick={() => setIsFilterOpen(false)}
                    ...
                    <tbody className={`divide-y ${T.divider}`}>
                      {filteredCustomers.length > 0 ? (
                        filteredCustomers.map((row, i) => (
                          <tr key={row.CardCode || i} className={`transition-colors duration-75 ${T.row}`}>
                ...
                {filteredCustomers.length > 0 && (
                  <div className={`flex-shrink-0 flex items-center justify-between px-6 py-2.5 border-t ${
                    ...
                    <p className={`text-[11px] ${T.ts}`}>
                      Showing all{' '}
                      <span className={`font-bold ${T.tp}`}>{filteredCustomers.length.toLocaleString()}</span>{' '}
                    ...
                    <p className={`text-[11px] ${T.tm}`}>
                      {customers.length.toLocaleString()} total in database
                    </p>
                  </div>
                )}
              </>
...
export default Vcp_CustomerRecords;