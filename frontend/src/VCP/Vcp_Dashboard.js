...
import VcpQuotaPerformance from '../components/Dashboard/StatusSummary/VcpQuotaPerformance';
...
// Helper function to calculate progress from total achieved
  const calculateCustomerProgress = async (customer) => {
    try {
      ...
      if (totalAchievedResponse.ok) {
        ...
        if (result.success) {
          ...
          // Return the updated customer object with progress calculated
          return {
            ...customer,
    ...
    // Return the original customer if there's an error
    return customer;
  };
    ...
    return sortedPayouts.map((payout) => {
      ...
      return {
        ...
        CalculationNote: carryOverBalance > 0 ? 
          `Original: ₱${payout.Amount.toFixed(2)} + Previous Monthly Balance: ₱${(adjustedAmount - payout.Amount).toFixed(2)} = ₱${adjustedAmount.toFixed(2)}` :
          `Monthly: ₱${payout.Amount.toFixed(2)}`
      };
    });
  };
...
const filteredCustomers = customers
  .filter((c) => {
    ...
    // matchesProgramStatus is removed – no longer filter here
    ...
    return matchesAgent && matchesRebateType && matchesProgressStatus &&
           matchesRebateStatus && matchesMinAmount && matchesMaxAmount &&
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    // Show page immediately after metrics + rebates load
    setLoading(false);
  }

  // Load customer status AFTER page is already visible (non-blocking)
  loadCustomerStatus();
};
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.success) {
        ...
        // If backend returns empty array, reflect that immediately
        ...
        // API returned success:false — treat as empty
        setCustomers([]);
        setAgents([]);
      }

    } else {
      // HTTP error — treat as empty
      setCustomers([]);
      setAgents([]);
    }
...
const loadDetailedTransactionsData = async (autoLoad = true) => {
  ...
  const loadDetailedPayoutsData = async (autoLoad = true) => {
    ...
    try {
      ...
      if (data.success) {
        console.log('Payouts loaded from database:', {
          count: data.data.payouts?.length || 0,
          customer: modalCustomer.code,
          rebateCode: modalCustomer.rebateCode,
          rebateType: modalCustomer.rebateType,
          isPercentage: modalCustomer.rebateType === 'Percentage'
        });
        
        const processedPayouts = (data.data.payouts || []).map(payout => {
          const baseAmount = parseFloat(payout.BaseAmount) || 0;
          const totalAmount = parseFloat(payout.TotalAmount) || parseFloat(payout.Amount) || 0;
          const amountReleased = parseFloat(payout.AmountReleased) || 0;
          ...
          // ADD THIS FOR PERCENTAGE REBATES:
          if (modalCustomer.rebateType === 'Percentage') {
            // Add percentage-specific fields from backend response
        ...
        // Save previous balance into modalCustomer so VcpPayoutHistory can access it
        setModalCustomer(prev => ({
          ...prev,
        ...
        console.error('❌ Payout API returned error:', data.message);
        setDetailedPayouts([]);
      }
    } catch (error) {
...
// Call this when transaction tab is clicked for debugging
// You can add a button or call it in handleTabChange
...
useEffect(() => {
  ...
  const handlePayoutStatusChange = async (payoutId, newStatus) => {
    try {
      ...
      const response = await fetch(`http://192.168.100.193:3009/api/vcp/payouts/payouts/${payoutId}/status`, {
        method: 'PUT',
        headers: {
  ...
  const handleAmountReleasedChange = async (payoutId, amount) => {
    try {
      ...
      const response = await fetch(`http://192.168.100.193:3009/api/vcp/payouts/payouts/${encodeURIComponent(payoutId)}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
  ...
  const clearAllFilters = () => {
    ...
    setSelectedProgramStatus("All");  // ← ADD THIS
    setMinRebateAmount("");
    setMaxRebateAmount("");
  ...
  const handleSaveCustomer = async (customerCode) => {
    try {
      ...
      console.log('📤 Sending request to backend:', {
        url: `http://192.168.100.193:3009/api/vcp/dashboard/rebate/customer?db=${currentDatabase}`,
        method: 'PUT',
        body: requestBody
      });

      const response = await fetch(`http://192.168.100.193:3009/api/vcp/dashboard/rebate/customer?db=${currentDatabase}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
...
const handleSaveItem = async (itemCode) => {
  try {
    ...
    const response = await fetch(`http://192.168.100.193:3009/api/vcp/dashboard/rebate/item?db=${currentDatabase}`, {
      method: 'PUT',
  ...
  const handleStatusToggle = async (rebateCode, statusValue) => {
    try { 
      ...
      const response = await fetch(`http://192.168.100.193:3009/api/vcp/dashboard/rebates?db=${currentDatabase}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
