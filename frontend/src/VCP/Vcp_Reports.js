          ...
          >
            {isFirstPage && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                <img src={vcpReport} alt="Vcp" style={{ width: '160px', height: 'auto' }} />
...
function Vcp_Reports() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/vcp/generate-multi-customer-report`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify({ customerCodes, dateFrom: dateFrom || null, dateTo: dateTo || null, db: 'VCP' }),
      });
      ...
      const headerHTML = `
        <div style="margin-bottom:15px;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:15px;">
            <div style="font-size:16pt;font-weight:bold;color:#000;">VCP CORPORATION</div>
            <div>
      ...
      Object.entries(customerMap).forEach(([code, groups], ci) => {
        ...
        body += `
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;">
            <div style="font-size:18pt;font-weight:bold;color:#000;">VCP CORPORATION</div>
  ...
  <div className="flex flex-col items-center justify-center min-h-[400px]">
    <div className="flex flex-col items-center space-y-4">
      <div className={`w-8 h-8 border-2 rounded-full animate-spin
        ${theme === 'dark' ? 'border-gray-600 border-t-blue-500' : 'border-gray-300 border-t-blue-600'}`}
      />
...
const renderAccessDenied = () => (
  <div className="flex flex-col items-center justify-center min-h-[400px] p-8">
    <div className="flex flex-col items-center max-w-md text-center space-y-6">
      <Lock size={32} className={theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} />
      
      <div className="space-y-2">
        <h2 className={`text-base font-medium ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>
          You don't have permission to view this page
        </h2>
        
        <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
          If you have access to multiple Orchestrator tenants, try switching to another from the profile icon. 
          Else, contact your administrator for necessary permissions.
        </p>
      </div>
                        ...
                        <div className={`rounded-xl border p-6 ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                          ...
                          <div className="flex justify-center overflow-x-auto">
                            ...
                            >
                              {isFirstPage && (
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                                  <img src={vcpReport} alt="Vcp Report" style={{ width: '160px', height: 'auto' }} />
                                  <div style={{ textAlign: 'right' }}>
                                    <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#1a1a1a', marginBottom: '4px' }}>KITANEX</div>
                                    <div style={{ fontSize: '11px', color: '#555', marginBottom: '2px' }}>{getRebatePeriod()}</div>
                                    <div style={{ fontSize: '11px', color: '#777' }}>{getCurrentDate()}</div>
