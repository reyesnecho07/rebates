...
function Nexchem_RebateSetup() {
  ...
  const fetchSalesEmployees = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://192.168.100.193:5000/api/nexchem/sales-employees");
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://192.168.100.193:5000/api/nexchem/items");
  ...
  const handleExportExcel = (type) => {
    try {
      ...
      if (type === 'customer') {
        ...
        showToast(`Customer data exported successfully${quotaType === "withQuota" ? " with aligned quotas" : ""}!`, "success");
            ...
            {activeTab === "Customer" && (
              <div className="mb-8">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-bold text-gray-800">Customer {quotaType === "withQuota" ? "Quota" : "Rebate"} Setup</h3>
