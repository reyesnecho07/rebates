...
// ─── Auto-detect Unit of Measure from item name ───────────────────────────────
...
function Nexchem_RebateSetup() {
    ...
    reader.onload = (e) => {
      try {
          ...
          if (jsonData.length > 0) {
            for (let i = 0; i < jsonData.length; i++) {
              ...
              if (Array.isArray(row)) {
                ...
                if (rowString.includes('rebate code') &&
                    rowString.includes('sales employee') &&
                    rowString.includes('date from')) {
                  if (i + 1 < jsonData.length) {
                    const valueRow = jsonData[i + 1];
                    headerInfo = {
                      rebateCode:    valueRow[0] || "",
                      salesEmployee: valueRow[1] || "",
                      dateFrom:      valueRow[2] || "",
                      dateTo:        valueRow[3] || "",
                      frequency:     valueRow[4] || "",
                      quotaType:     valueRow[5] || "",
                      rebateType:    valueRow[6] || ""
                    };
                    ...
                    if (headerInfo.quotaType) {
                      const qtv = headerInfo.quotaType.toLowerCase();
                      setQuotaType(qtv.includes('without') ? "withoutQuota" : "withQuota");
                    }
                    if (headerInfo.rebateType) {
                      const rtv = headerInfo.rebateType.toLowerCase();
                      if (rtv.includes('fixed'))            setRebateType("Fixed");
                      else if (rtv.includes('incremental')) setRebateType("Incremental");
                      else if (rtv.includes('percentage'))  setRebateType("Percentage");
                    }
                  }
                  break;
                }
        ...
        const customerSheetName = workbook.SheetNames.find(name =>
          name.toLowerCase() === 'customers' || name.toLowerCase().includes('customer')
        );
        if (customerSheetName) {
          ...
          if (customerJsonData.length > 0) {
            let headerRowIndex = -1, codeColIndex = -1, nameColIndex = -1;
            for (let i = 0; i < customerJsonData.length; i++) {
              const row = customerJsonData[i];
              if (Array.isArray(row)) {
                for (let j = 0; j < row.length; j++) {
                  const cell = row[j];
                  if (cell && typeof cell === 'string') {
                    const cellLower = cell.toLowerCase().trim();
                    if (cellLower.includes('customer code')) { codeColIndex = j; headerRowIndex = i; }
                    if (cellLower.includes('customer name')) { nameColIndex = j; headerRowIndex = i; }
                  }
                }
              }
              if (headerRowIndex !== -1 && codeColIndex !== -1 && nameColIndex !== -1) break;
            }
            if (headerRowIndex !== -1 && codeColIndex !== -1 && nameColIndex !== -1) {
              for (let i = headerRowIndex + 1; i < customerJsonData.length; i++) {
                const row = customerJsonData[i];
                if (Array.isArray(row)) {
                  const codeValue = row[codeColIndex] !== undefined ? String(row[codeColIndex]).trim() : "";
                  const nameValue = row[nameColIndex] !== undefined ? String(row[nameColIndex]).trim() : "";
                  if (codeValue || nameValue) {
                    const customerExists = customersDropdown.some(c =>
                      c.CardCode === codeValue || c.CardName === nameValue
                    );
                    if (customerExists) {
              ...
              if (importedCustomers.length > 0) {
                ...
                importedCustomers.forEach((_, index) => { initialEditingState[index] = false; });
        ...
        if (itemSheetName) {
          ...
          if (itemJsonData.length > 0) {
            let headerRowIndex = -1, codeColIndex = -1, nameColIndex = -1,
                qtyColIndex = -1, rebateColIndex = -1;
            for (let i = 0; i < itemJsonData.length; i++) {
              const row = itemJsonData[i];
              if (Array.isArray(row)) {
                for (let j = 0; j < row.length; j++) {
                  const cell = row[j];
                  if (cell && typeof cell === 'string') {
                    const cellLower = cell.toLowerCase().trim();
                    if (cellLower.includes('item code'))    { codeColIndex   = j; headerRowIndex = i; }
                    if (cellLower.includes('item name'))    { nameColIndex   = j; headerRowIndex = i; }
                    if (cellLower === 'qty' || cellLower.includes('quantity')) { qtyColIndex = j; headerRowIndex = i; }
                    if (cellLower.includes('rebate per bag') || cellLower.includes('percentage per bag')) {
                      rebateColIndex = j; headerRowIndex = i;
                    }
                  }
                }
              }
            ...
            if (headerRowIndex !== -1 && codeColIndex !== -1 && nameColIndex !== -1) {
              for (let i = headerRowIndex + 1; i < itemJsonData.length; i++) {
                const row = itemJsonData[i];
                if (Array.isArray(row)) {
                  const codeValue   = row[codeColIndex] !== undefined ? String(row[codeColIndex]).trim() : "";
                  const nameValue   = row[nameColIndex] !== undefined ? String(row[nameColIndex]).trim() : "";
                  const qtyValue    = (qtyColIndex    !== -1 && row[qtyColIndex]    !== undefined) ? String(row[qtyColIndex]).trim()    : "";
                  const rebateValue = (rebateColIndex !== -1 && row[rebateColIndex] !== undefined) ? String(row[rebateColIndex]).trim() : "";
                  if (codeValue || nameValue) {
                    ...
                    if (itemExists) {
                      importedItems.push({
                        ...
                        name:             nameValue,
                        unitPerQty:       qtyValue,
                        rebatePerBag:     rebateType !== "Percentage" ? rebateValue : "",
                        percentagePerBag: rebateType === "Percentage" ? rebateValue : "",
                        ranges:           {}
                      });
                    }
                  }
              ...
              if (importedItems.length > 0) {
                ...
                const initialEditingState = {};
                importedItems.forEach((_, index) => { initialEditingState[index] = false; });
  ...
  const processItemDataSimple = (excelData) => {
    try {
      ...
      dataRows.forEach((row) => {
        ...
        const iName = itemData?.ItemName || nameValue;
        importedItems.push({
          ...
          unitOfMeasure:    detectUnitOfMeasure(iName),
          ranges:           {},
        });
      });
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      const headerData = [
        ["Rebate Code", "Sales Employee", "Date From", "Date To", "Frequency", "Quota Type", "Rebate Type"],
        [rebateCode, selectedSalesEmployee, selectedDateFrom, selectedDateTo, selectedFrequency,
         quotaType === "withQuota" ? "With Quota" : "Without Quota", rebateType],
        []
      ];
      const customerData = customers
        .filter(c => c.code || c.name)
        .map(c => [c.code || '', c.name || '']);
      const customerSheetData = [...headerData, ["Customer Code", "Customer Name"], ...customerData];
      const customerWorksheet = XLSX.utils.aoa_to_sheet(customerSheetData);
      customerWorksheet['!cols'] = [
      ...
      const itemsSheetData = [...headerData, itemsHeader, ...itemsData];
      const itemsWorksheet = XLSX.utils.aoa_to_sheet(itemsSheetData);
      itemsWorksheet['!cols'] = [
        { wch: 15 }, { wch: 25 }, { wch: 12 }, { wch: 12 },
        { wch: 12 }, { wch: 15 }, { wch: 15 }, { wch: 5 },
      ...
      showToast(`Data exported successfully to both Customers and Items tabs!`, "success");
    } catch (error) {
      ...
      if (index !== null) {
        ...
        showToast(`Item "${selected.ItemName || selected.name}" updated successfully`, "success");
            ...
            if (addedCount > 0) {
              ...
              showToast("No new items were added (they may already exist in the list)", "info");
            }
          }
        }
...
const handleUpdate = async () => {
  ...
  try {
    ...
    if (existingCustRes.ok) {
      const custData = await existingCustRes.json();
      existingCustomers = custData.customers || [];
    }
    if (existingItemsRes.ok) {
      const itemData = await existingItemsRes.json();
      existingItems = itemData.items || [];
    }

    // Create maps: key = customer code, value = original CreatedDate
    ...
    // 2️⃣ Update the program header (this already does not touch CreatedDate)
    const salesEmployee = salesEmployees.find(emp => emp.SlpName === selectedSalesEmployee);
...
// ── saveFixedRebateData ──────────────────────────────────────────────────────
// Bug 1 fix: default empty objects so new saves don't crash
const saveFixedRebateData = async (rebateCodeId, database, customerCreatedMap = {}, itemCreatedMap = {}) => {
  for (const customer of customers) {
    ...
    const originalCreated = customerCreatedMap[customer.code] || null;   // ✅ safe now
    ...
    if (!custRes.ok) throw new Error(`Failed to save customer ${customer.code}`);
    ...
    if (quotaType === "withQuota" && customer.quotas && customer.quotas.length > 0) {
      ...
      for (let i = 0; i < monthlyPeriods.length; i++) {
        ...
        if (targetQty !== "" && targetQty !== null) {
          ...
          if (!isNaN(val)) {
            await fetch(`${API_BASE}/fix-cust-quota?db=${database}`, {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ CustRebateId: custRebateId, Month: monthName, TargetQty: val }),
            });
          }
        }
      }
  ...
  for (const item of items) {
    ...
    const originalCreated = itemCreatedMap[item.code] || null;           // ✅ safe now
    const ir = await fetch(`${API_BASE}/fix-prod-rebate?db=${database}`, {
      ...
      body: JSON.stringify({
        ...
        UnitOfMeasure: item.unitOfMeasure || '',     // ✅ already correct here, kept
        CreatedDate:   originalCreated,
      }),
    });
    if (!ir.ok) throw new Error(`Failed to save item ${item.code}`);
  }
};

// ── saveIncrementalRebateData ────────────────────────────────────────────────
// Bug 2 + Bug 4 fix: accept maps, add UnitOfMeasure, preserve CreatedDate
const saveIncrementalRebateData = async (rebateCodeId, database, customerCreatedMap = {}, itemCreatedMap = {}) => {
  for (const customer of customers) {
    if (!customer.code || !customer.name) continue;
    const originalCreated = customerCreatedMap[customer.code] || null;   // Bug 4 fix
    const custRes = await fetch(`${API_BASE}/inc-cust-rebate?db=${database}`, {
      ...
      body: JSON.stringify({
        ...
        CardName:    customer.name,
        QtrRebate:   customer.qtrRebate || 0,
        CreatedDate: originalCreated,            // Bug 4 fix
      }),
    });
      ...
      for (const [, ranges] of Object.entries(customer.ranges)) {
        ...
        for (const range of ranges) {
          const rr = await fetch(`${API_BASE}/inc-cust-range?db=${database}`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              IncCustRebateId: incCustRebateId, RangeNo: rangeNo,
              MinQty:          parseInt(range.min)    || 0,
              MaxQty:          parseInt(range.max)    || 0,
              RebatePerBag:    parseFloat(range.rebate) || 0,
            }),
          });
  ...
  for (const item of items) {
    if (!item.code || !item.name) continue;
    const originalCreated = itemCreatedMap[item.code] || null;           // Bug 4 fix
    const ir = await fetch(`${API_BASE}/inc-item-rebate?db=${database}`, {
      ...
      body: JSON.stringify({
        ...
        ItemCode:      item.code,
        ItemName:      item.name,
        UnitPerQty:    parseInt(item.unitPerQty) || 0,
        UnitOfMeasure: item.unitOfMeasure || '',  // ✅ Bug 2 fix — was missing
        CreatedDate:   originalCreated,            // Bug 4 fix
      }),
    });
      ...
      for (const [, ranges] of Object.entries(item.ranges)) {
        ...
        for (const range of ranges) {
          const rr = await fetch(`${API_BASE}/inc-item-range?db=${database}`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ItemRebateId: itemRebateId, RangeNo: rangeNo,
              MinQty:       parseInt(range.min)    || 0,
              MaxQty:       parseInt(range.max)    || 0,
              RebatePerBag: parseFloat(range.rebate) || 0,
            }),
...
// ── savePercentageRebateData ─────────────────────────────────────────────────
// Bug 3 + Bug 4 fix: accept maps, add UnitOfMeasure, preserve CreatedDate
// ── savePercentageRebateData ─────────────────────────────────────────────────
// Bug 3 + Bug 4 fix: accept maps, add UnitOfMeasure, preserve CreatedDate
    ...
    if (quotaType === "withQuota" && customer.percentages && customer.percentages.length > 0) {
      ...
      if (percentagesToSave.length > 0) {
        const bulkRes = await fetch(`${API_BASE}/per-cust-quotas/bulk?db=${database}`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ CustRebateId: perCustRebateId, quotas: percentagesToSave }),
        });
        if (!bulkRes.ok) {
          for (const p of percentagesToSave) {
            await fetch(`${API_BASE}/per-cust-quota?db=${database}`, {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ PerCustRebateId: perCustRebateId, Month: p.Month, TargetQty: p.TargetQty }),
            });
          }
        }
      }
  ...
  for (const item of items) {
    ...
    const ir = await fetch(`${API_BASE}/per-prod-rebate?db=${database}`, {
      ...
      body: JSON.stringify({
        ...
        UnitOfMeasure:    item.unitOfMeasure || '',           // ✅ Bug 3 fix — was missing
        CreatedDate:      originalCreated,                    // Bug 4 fix
      }),
    });
    if (!ir.ok) { const t = await ir.text(); throw new Error(`Failed item: ${t}`); }
                                            ...
                                            <div className="w-36">
                                              {isRowEditable('item', idx) ? (
                                                ...
                                                <span className={`text-sm font-medium ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                                                  {item.rebatePerBag
                                                    ? `₱${item.rebatePerBag}${item.unitOfMeasure ? `/${item.unitOfMeasure}` : '/bag'}`
                                                    : "—"}
                                                </span>
                                              )}
                                            </div>
