...
function Nexchem_ItemRecords() {
  ...
  // Fetch items from local NEXCHEM_OWN database
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await fetch("http://192.168.100.193:3006/api/sync/local/items?db=NEXCHEM_OWN");
        ...
        setItems(itemsData);
        console.log(`Loaded ${itemsData.length} items from NEXCHEM_OWN database`);
      } catch (err) {
