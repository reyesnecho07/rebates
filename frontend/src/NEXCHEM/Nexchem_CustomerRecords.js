...
const componentMetadata = {
  name: 'Nexchem_CustomerRecords',
  version: '2.0.0',
  description: 'Displays and manages a list of customer records.',
  routePath: '/Nexchem_CustomerRecords',
};

function Nexchem_CustomerRecords() {
  const { theme, updateTheme } = useTheme();
  const location = useLocation();
  const routePath = '/Nexchem_CustomerRecords';
  ...
  useEffect(() => {
    ...
    const fetchCustomers = async () => {
      try {
        const res = await fetch("http://192.168.100.193:3009/api/sync/local/customers?db=NEXCHEM_OWN");
...
export default Nexchem_CustomerRecords;