...
function Nexchem_CustomerRecords() {
  ...
  // Fetch customers from local NEXCHEM_OWN database
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const res = await fetch("http://192.168.100.193:3006/api/sync/local/customers?db=NEXCHEM_OWN");
        ...
        if (Array.isArray(data)) {
          ...
          customersData = Array.isArray(data.data) ? data.data : [data.data];
        } else {
        ...
        console.log(`Loaded ${customersData.length} customers from NEXCHEM_OWN database`);
      } catch (err) {
        ...
        <div className="pt-16 flex-1 p-8 overflow-auto">
          ...
          <div className={`flex items-center gap-3 mb-6 pb-4 border-b ${
            ...
            <div>
              ...
              }`}>Manage and view all customer information from NEXCHEM_OWN database</p>
            </div>
          </div>
            {/* Search Bar */}
                            ...
                            <td className="px-4 py-3 font-medium">
                              <div className="flex items-center gap-2">
                                <div className={`w-7 h-7 rounded-lg flex items-center justify-center font-semibold text-xs ${
                                  ...
                                  {row.CardCode ? row.CardCode.charAt(0) : "C"}
                                </div>
                            ...
                            <div className={`text-center py-6 px-4 rounded-xl border shadow-sm ${
                              ...
                              <div className="space-y-2 mb-4">
                                <h3 className={`text-lg font-bold ${
                                  ...
                                  {customers.length === 0 ? "No Customers" : "No Customers Found"}
                                </h3>
                                <p className={`max-w-md mx-auto leading-relaxed text-xs ${
                                  theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
                                }`}>
                                  {customers.length === 0 
                                    ? "No customers found in the database. The database might be empty or there was an error loading customers." 
                ...
                <div className="flex justify-between items-center mt-4 flex-wrap gap-3">
                  <div className={`text-xs ${
                    ...
                    Showing <span className={`font-semibold ${
                      ...
                      theme === 'dark' ? 'text-blue-400' : 'text-blue-600'
                    }`}>{Math.min(currentPage * rowsPerPage, filteredCustomers.length)}</span> of <span className={`font-semibold ${
                      theme === 'dark' ? 'text-blue-400' : 'text-blue-600'
                    }`}>{filteredCustomers.length}</span> customers
                  </div>
