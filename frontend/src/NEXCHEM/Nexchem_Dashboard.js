      ...
      <table className="w-full">
        <thead className={`sticky top-0 border-b ${isDark ? 'bg-gradient-to-r from-gray-800 to-gray-900 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
          <tr>
            ...
            <th className={`px-3 py-2 text-center text-xs font-semibold uppercase tracking-wider min-w-[80px] max-w-[80px] ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>Actions</th>
          </tr>
        </thead>
        <tbody className={`divide-y ${isDark ? 'divide-gray-700/50' : 'divide-gray-100'}`}>
          {rebateDetails.items.map((item, idx) => {
            ...
            return (
              <tr key={idx} className={`transition-colors duration-150 ${isDark ? 'hover:bg-gray-700/30' : 'hover:bg-gray-50/50'}`}>
                  ...
                  <div className="flex items-center gap-2 w-full">
                    <div className={`w-5 h-5 rounded-md flex items-center justify-center shadow-sm flex-shrink-0 ${isDark ? 'bg-gradient-to-br from-blue-900/30 to-blue-900/30' : 'bg-gradient-to-br from-blue-100 to-blue-200'}`}>
                      <Blocks size={10} className={isDark ? 'text-blue-400' : 'text-blue-600'} />
                        ...
                        <div className="min-w-0 flex-1">
                          <div
                            className={`font-medium text-xs truncate overflow-hidden text-ellipsis whitespace-nowrap ${
                              isDark ? 'text-gray-200' : 'text-gray-800'
                            }`}
                          >
                            {item.description}
                ...
                <td className="px-3 py-2 align-top">
                  <code className={`text-xs px-1.5 py-0.5 rounded font-medium truncate inline-block max-w-full ${isDark ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>
                    {item.code}
                  </code>
                </td>
                <td className="px-3 py-2 text-center align-top">
                  {editingItems[item.code] ? (
                    <input type="number" value={item.unitPerQty} onChange={(e) => handleItemChange(item.code, 'unitPerQty', e.target.value)}
                      className={`w-12 px-1 py-1 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} />
                  ) : (
                    <span className={`text-xs px-2 py-1 rounded font-medium inline-block text-center min-w-10 ${isDark ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-900'}`}>
                      {item.unitPerQty}
                    </span>
                  )}
                </td>
                ...
                <td className="px-3 py-2 text-center align-top">
                  {editingItems[item.code] ? (
                    <input type="text" value={item.uom || ''} onChange={(e) => handleItemChange(item.code, 'uom', e.target.value)}
                      className={`w-20 px-1 py-1 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} />
                  ) : (
                    <span className={`text-xs px-2 py-1 rounded font-medium inline-block text-center min-w-10 ${isDark ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-900'}`}>
                      {item.uom || '—'}
                    </span>
                ...
                <td className="px-3 py-2 text-center align-top">
                  {editingItems[item.code] ? (
                    <div className="flex items-center gap-0.5 justify-center">
                      ...
                      <input type="number" step="0.01" value={rebatePerBagValue} onChange={(e) => handleItemChange(item.code, 'rebatePerBag', e.target.value)}
                        className={`w-12 px-1 py-1 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} />
                    </div>
                  ) : (
                    <span className={`text-xs px-2 py-1 rounded font-medium inline-block text-center min-w-10 ${isDark ? 'bg-emerald-500/80 text-white' : 'bg-emerald-500 text-white'}`}>
                ...
                <td className="px-3 py-2 text-center align-top">
                  {editingItems[item.code] ? (
                    <div className="flex gap-1 justify-center">
                      <button onClick={() => handleSaveItem(item.code)} className="p-1 bg-green-500 text-white rounded hover:bg-green-600 transition-colors" title="Save"><Check size={10} /></button>
                      <button onClick={() => handleCancelEditItem(item.code)} className="p-1 bg-red-500 text-white rounded hover:bg-red-600 transition-colors" title="Cancel"><X size={10} /></button>
                    </div>
                  ) : (
                    <button onClick={() => access?.canEdit && handleEditItem(item.code)} disabled={!access?.canEdit}
                      className={`p-1 rounded transition-colors ${access?.canEdit ? 'bg-blue-500 text-white hover:bg-blue-600 cursor-pointer' : 'bg-gray-300 text-gray-400 cursor-not-allowed dark:bg-gray-700 dark:text-gray-600'}`}>
                      <Edit size={10} />
                    </button>
                  )}
                </td>
            ...
            <tr key={item.code} className={`transition-colors duration-150 ${isDark ? 'hover:bg-gray-700/30' : 'hover:bg-gray-50/50'}`}>
              <td className="px-3 py-2 align-top">
                <div className="flex items-center gap-2 w-full">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className={`font-medium text-xs truncate overflow-hidden text-ellipsis whitespace-nowrap ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
                      {item.description}
                    </div>
                  </div>
                </div>
              ...
              <td className="px-3 py-2 text-center align-top">
                {editingItems[item.code] ? (
                  <input type="number" value={item.unitPerQty} onChange={(e) => handleItemChange(item.code, 'unitPerQty', e.target.value)}
                    className={`w-14 px-1 py-1 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} />
                ) : (
                  <span className={`text-xs px-2 py-1 rounded font-medium inline-block text-center min-w-10 ${isDark ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-900'}`}>
                    {item.unitPerQty}
                  </span>
                    ...
                    return (
                      <div key={`${item.code}-${rangeIndex}`} className="flex flex-col items-center p-1">
                        ...
                        <div className="flex items-center gap-0.5">
                          {editingItems[item.code] ? (
                            <>
                              <input type="number" value={range.minQty || 0} onChange={(e) => handleItemRangeChange(item.code, rangeIndex, 'minQty', e.target.value)}
                                className={`w-10 px-1 py-0.5 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} title="Min Qty" />
                              <span className={`text-xs ${colors.label} px-0.5`}>–</span>
                              <input type="number" value={range.maxQty || 0} onChange={(e) => handleItemRangeChange(item.code, rangeIndex, 'maxQty', e.target.value)}
                                className={`w-10 px-1 py-0.5 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} title="Max Qty" />
                            </>
                          ) : (
                            <>
                              <span className={`text-xs px-2 py-0.5 rounded-l font-medium ${colors.text} bg-opacity-10 ${colors.badge.replace('text-', 'bg-')}`}>
                    ...
                    return (
                      <div key={`${item.code}-rebate-${rangeIndex}`} className="flex flex-col items-center p-1">
                        ...
                        {editingItems[item.code] ? (
                          <div className="flex items-center gap-0.5">
                            <span className={`text-xs ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>₱</span>
                            <input type="number" value={range.rebatePerBag || 0} onChange={(e) => handleItemRangeChange(item.code, rangeIndex, 'rebatePerBag', e.target.value)}
                              className={`w-12 px-1 py-0.5 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} />
                          </div>
                        ) : (
                          <span className={`text-xs px-2.5 py-0.5 rounded font-medium ${colors.text} bg-opacity-10 ${colors.badge.replace('text-', 'bg-')}`}>
                            ₱{range.rebatePerBag || 0}
        ...
        <tbody className={`divide-y ${isDark ? 'divide-gray-700/50' : 'divide-gray-100'}`}>
          {rebateDetails.items.map((item, idx) => (
            <tr key={idx} className={`transition-colors duration-150 ${isDark ? 'hover:bg-gray-700/30' : 'hover:bg-gray-50/50'}`}>
              ...
              <td className="px-3 py-2 text-center align-top">
                {editingItems[item.code] ? (
                  <input type="number" value={item.unitPerQty || 1} onChange={(e) => handleItemChange(item.code, 'unitPerQty', e.target.value)}
                    className={`w-12 px-1 py-1 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} />
                ) : (
                  <span className={`text-xs px-2 py-1 rounded font-medium inline-block text-center min-w-10 ${isDark ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-900'}`}>
                    {item.unitPerQty || 1}
              ...
              <td className="px-3 py-2 text-center align-top">
                {editingItems[item.code] ? (
                  <div className="flex items-center gap-0.5 justify-center">
                    <input type="number" value={item.percentage || 0} onChange={(e) => handleItemChange(item.code, 'percentage', e.target.value)}
                      className={`w-12 px-1 py-1 rounded text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent ${isDark ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'}`} />
                    <span className={`text-xs ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>%</span>
                  </div>
                ) : (
              ...
              <div className={`flex-shrink-0 border-b px-6 py-4 ${
                ...
                <div className="grid grid-cols-5 gap-2.5">
                  ...
                  <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${
                    theme === 'dark' ? 'bg-rose-900/20 border-rose-700/30' : 'bg-rose-50 border-rose-200'
                  }`}>
                  ...
                  {/* Sales Employee */}
                  <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${
                    theme === 'dark' ? 'bg-amber-900/20 border-amber-700/30' : 'bg-amber-50 border-amber-200'
                  }`}>
                  ...
                  {/* Period */}
                  <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${
                    theme === 'dark' ? 'bg-emerald-900/20 border-emerald-700/30' : 'bg-emerald-50 border-emerald-200'
                  }`}>
