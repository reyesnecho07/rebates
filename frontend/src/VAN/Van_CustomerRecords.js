...
function Van_CustomerRecords() {
  ...
  // Fetch customers from local VAN_OWN database
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const res = await fetch("http://192.168.100.193:3006/api/sync/local/customers?db=VAN_OWN");
        ...
        console.log(`Loaded ${customersData.length} customers from VAN_OWN database`);
      } catch (err) {
          ...
          <div className={`bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-3xl border ${
            ...
            <div className={`flex items-center gap-3 mb-6 pb-4 border-b ${
              ...
              <div>
                ...
                }`}>Manage and view all customer information from VAN_OWN database</p>
              </div>
            </div>
