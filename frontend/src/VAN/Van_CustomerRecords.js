...
import { Users, Search, Filter, X, Check, Lock } from "lucide-react";
...
const componentMetadata = {
  name: 'Van_CustomerRecords',
  version: '2.0.0',
  description: 'Displays and manages a list of customer records.',
  routePath: '/Van_CustomerRecords',
};

function Van_CustomerRecords() {
  const { theme, updateTheme } = useTheme();
  const location = useLocation();
  const routePath = '/Van_CustomerRecords';
  ...
  useEffect(() => {
    ...
    const fetchCustomers = async () => {
      try {
        const res = await fetch("http://192.168.100.193:3009/api/sync/local/customers?db=VAN_OWN");
          ...
          <div className={`flex-1 flex flex-col rounded-2xl border shadow-sm w-full max-w-[1600px] mx-auto mt-4 overflow-hidden ${
            ...
            {accessLoading ? (
              ...
              <>
                ...
                <div className={`flex-shrink-0 flex flex-wrap items-center gap-2 px-6 py-2 border-b ${
...
export default Van_CustomerRecords;