...
const SearchRebateModal = ({ isOpen, onClose, searchCode, setSearchCode, onSearch, searchLoading, searchError, theme }) => {
  ...
  useEffect(() => {
    ...
    fetch('http://192.168.100.193:3006/api/rebate-program/all-codes?db=VAN_OWN')
      .then(r => r.json())
...
function Van_RebateSetup() {
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      const fileName  = `REBATE_${rtF}_${dateFromF}_${dateToF}_VAN.xlsx`;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const rebateProgramData = {
        ...
        QuotaType: quotaType === "withQuota" ? "With Quota" : "Without Quota", db: 'VAN_OWN',
      };
  ...
  const saveFixedRebateData = async (rebateCodeId, database) => {
          ...
          if (targetQty !== "" && targetQty !== null) {
            ...
            if (!isNaN(val)) {
              const qr = await fetch(`${API_BASE}/fix-cust-quota?db=${database}`, {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ Id: nextQuotaId++, CustRebateId: custRebateId, Month: monthName, TargetQty: val, db: 'VAN_OWN' }),
              });
    ...
    for (const item of items) {
      ...
      const ir = await fetch(`${API_BASE}/fix-prod-rebate?db=${database}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ RebateCode: rebateCodeId, ItemCode: item.code, ItemName: item.name, UnitPerQty: parseFloat(item.unitPerQty) || 0, RebatePerBag: parseFloat(item.rebatePerBag) || 0, db: 'VAN_OWN' }),
      });
  ...
  const saveIncrementalRebateData = async (rebateCodeId, database) => {
    ...
    for (const item of items) {
      ...
      const ir = await fetch(`${API_BASE}/inc-item-rebate?db=${database}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ RebateCode: rebateCodeId, ItemCode: item.code, ItemName: item.name, UnitPerQty: parseInt(item.unitPerQty) || 0, db: 'VAN_OWN' }),
      });
  ...
  const savePercentageRebateData = async (rebateCodeId, database) => {
    for (const customer of customers) {
      ...
      const custRes = await fetch(`${API_BASE}/per-cust-rebate?db=${database}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ RebateCode: rebateCodeId, CardCode: customer.code, CardName: customer.name, db: 'VAN_OWN' }),
      });
      if (!custRes.ok) { const t = await custRes.text(); throw new Error(`Failed: ${t}`); }
      const custResult      = await custRes.json();
          ...
          if (!bulkRes.ok) {
            for (const p of percentagesToSave) {
              await fetch(`${API_BASE}/per-cust-quota?db=${database}`, {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ PerCustRebateId: perCustRebateId, Month: p.Month, TargetQty: p.TargetQty, db: 'VAN_OWN' }),
              });
            }
          }
        }
    ...
    for (const item of items) {
      ...
      const ir = await fetch(`${API_BASE}/per-prod-rebate?db=${database}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ RebateCode: rebateCodeId, ItemCode: item.code, ItemName: item.name, UnitPerQty: parseFloat(item.unitPerQty) || 0, PercentagePerBag: parseFloat(item.percentagePerBag) || 0, db: 'VAN_OWN' }),
      });
      if (!ir.ok) { const t = await ir.text(); throw new Error(`Failed item: ${t}`); }
    }
  };
