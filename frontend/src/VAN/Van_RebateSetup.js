...
import {
  ...
  Plus,
  Minus,
  RefreshCw // ADDED: Import RefreshCw icon
} from "lucide-react";
...
// Range Modal Component for Incremental Rebates with Quarterly Support
const RangeModal = ({ isOpen, onClose, customer, onSave, quotaPeriods, importedQuotas = [], rebateType = "Fixed", quotaType = "withQuota" }) => {
  const [localRanges, setLocalRanges] = useState({});
  const [activePeriod, setActivePeriod] = useState(0);

  useEffect(() => {
    if (customer && customer.ranges) {
      // Initialize ranges for all periods if they don't exist
      ...
      quotaPeriods.forEach((_, periodIndex) => {
        if (customer.ranges[periodIndex]) {
          ...
          // Initialize with default ranges for this period
      ...
      // Initialize with default ranges for all periods
      const defaultRanges = {};
      quotaPeriods.forEach((_, periodIndex) => {
  ...
  const handleRangeChange = (periodIndex, rangeIndex, field, value) => {
    ...
    if (value === "" || /^-?\d*\.?\d*$/.test(value)) {
      ...
      // Auto-adjust subsequent min values within the same period
  ...
  const removeRange = (periodIndex, rangeIndex) => {
    const newRanges = { ...localRanges };
    if (newRanges[periodIndex].length <= 1) {
      return; // Keep at least one range per period
    }
  ...
  // Copy ranges from one period to another
  const copyRangesToPeriod = (sourcePeriodIndex, targetPeriodIndex) => {
    ...
    setLocalRanges(newRanges);
    showToast(`Ranges copied to ${quotaPeriods[targetPeriodIndex].period}`, "success");
  };

  // Copy ranges to all future periods
  const copyRangesToAllPeriods = (sourcePeriodIndex) => {
    ...
    setLocalRanges(newRanges);
    showToast(`Ranges copied to all future periods`, "success");
  };

  const showToast = (message, type) => {
    // Simple toast implementation for modal
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white rounded-2xl w-[95%] max-w-7xl max-h-[90vh] overflow-hidden relative shadow-2xl border-0">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-green-50 to-emerald-100 border-b border-green-200">
          ...
          <button
            onClick={onClose}
            className="p-2.5 rounded-xl bg-white/80 hover:bg-white transition-all duration-200 border border-green-200 hover:border-green-300"
          >
        ...
        <div className="p-8 max-h-[50vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-4">
            {localRanges[activePeriod]?.map((range, rangeIndex) => (
              <div key={rangeIndex} className="bg-white border border-green-100 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-gray-900 text-lg">Range {rangeIndex + 1}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      placeholder="Min"
                      className="w-full px-4 py-3 border border-green-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-400 bg-white transition-all duration-200 font-medium placeholder-gray-400"
                    />
                  </div>
          ...
          <div className="mt-6 flex justify-center">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-green-600 text-white hover:bg-green-700 rounded-xl transition-all duration-200 font-semibold shadow-lg hover:shadow-xl"
            >
        ...
        {/* Footer */}
        <div className="flex justify-between items-center p-8 border-t border-green-100 bg-green-50/30">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={handleSave}
              className="px-8 py-3 bg-green-600 text-white hover:bg-green-700 rounded-xl transition-all duration-200 font-semibold flex items-center gap-2 hover:scale-105 shadow-lg shadow-green-500/30"
            >
...
// Product Range Modal Component for Product Rebates
const ProductRangeModal = ({ isOpen, onClose, product, onSave, quotaPeriods, rebateType = "Fixed", quotaType = "withQuota" }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white rounded-2xl w-[95%] max-w-7xl max-h-[90vh] overflow-hidden relative shadow-2xl border-0">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-purple-50 to-purple-100 border-b border-purple-200">
          <div className="flex items-center gap-3">
          ...
          <button
            onClick={onClose}
            className="p-2.5 rounded-xl bg-white/80 hover:bg-white transition-all duration-200 border border-purple-200 hover:border-purple-300"
          >
            <X className="w-5 h-5 text-gray-600 hover:text-gray-800 transition-colors" />
          </button>
        </div>
        ...
        {/* Content */}
        <div className="p-8 max-h-[50vh] overflow-y-auto custom-scrollbar">
          <div className="mb-6 p-4 bg-gradient-to-r from-purple-50 to-purple-50 border border-purple-200 rounded-xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <TrendingUp className="w-5 h-5 text-purple-600" />
                <div>
          ...
          <div className="space-y-4">
            {localRanges[activePeriod]?.map((range, rangeIndex) => (
              <div key={rangeIndex} className="bg-white border border-purple-100 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-gray-900 text-lg">Range {rangeIndex + 1}</h3>
                  {localRanges[activePeriod].length > 1 && (
                    <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => handleRangeChange(activePeriod, rangeIndex, 'min', e.target.value)}
                      placeholder="Min"
                      className="w-full px-4 py-3 border border-purple-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-400 bg-white transition-all duration-200 font-medium placeholder-gray-400"
                    />
                  </div>
          ...
          <div className="mt-6 flex justify-center">
            <button
              onClick={() => addRange(activePeriod)}
              className="flex items-center gap-2 px-6 py-3 bg-purple-600 text-white hover:bg-purple-700 rounded-xl transition-all duration-200 font-semibold shadow-lg hover:shadow-xl"
            >
              <Plus className="w-4 h-4" />
              Add Another Range for {quotaPeriods[activePeriod]?.period}
            </button>
        ...
        {/* Footer */}
        <div className="flex justify-between items-center p-8 border-t border-purple-100 bg-purple-50/30">
          <div className="text-sm text-gray-600">
            {localRanges[activePeriod]?.length || 0} range{(localRanges[activePeriod]?.length || 0) !== 1 ? 's' : ''} configured for {quotaPeriods[activePeriod]?.period}
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={handleSave}
              className="px-8 py-3 bg-purple-600 text-white hover:bg-purple-700 rounded-xl transition-all duration-200 font-semibold flex items-center gap-2 hover:scale-105 shadow-lg shadow-purple-500/30"
            >
              <Save className="w-4 h-4" />
              Save All Ranges
            </button>
...
// Quota Modal Component for Fixed Rebates
const QuotaModal = ({ isOpen, onClose, customer, onSave, quotaPeriods, importedQuotas = [], rebateType = "Fixed", quotaType = "withQuota", selectedFrequency = "" }) => {
  ...
  useEffect(() => {
    if (customer && customer.quotas) {
      ...
      console.log('Quota periods length:', quotaPeriods.length);
      console.log('Imported quotas:', importedQuotas);
      
      // Always use existing customer quotas first if they exist
      if (customer.quotas.length > 0) {
        // Map customer quotas to the monthly periods
        ...
        // Use imported quotas if no customer quotas exist
      ...
      // Ensure we have entries for ALL monthly periods (fill empty for any missing)
      const monthlyPeriods = getMonthlyPeriodsFromQuotaPeriods();
      monthlyPeriods.forEach((_, index) => {
      ...
      console.log('Final local quotas:', initialQuotas);
      setLocalQuotas(initialQuotas);
    }
  }, [customer, importedQuotas, quotaPeriods]);

  // Create monthly periods from quarterly quota periods
    ...
    quotaPeriods.forEach((quarterPeriod) => {
      if (quarterPeriod.startDate && quarterPeriod.endDate) {
        ...
        while (currentDate <= endDate) {
          ...
          // Ensure we don't exceed the quarter end date
        ...
        // Fallback: if we don't have proper dates, create 3 months for the quarter
  ...
  const handleSave = () => {
    // Create a complete array with quotas for all monthly periods
    const monthlyPeriods = getMonthlyPeriodsFromQuotaPeriods();
    const quotasArray = monthlyPeriods.map((_, index) => localQuotas[index] || "");
    console.log('Saving quotas array:', quotasArray);
    onSave(quotasArray);
    onClose();
  };
  ...
  // Get display periods - show monthly breakdown for both frequencies
  const getDisplayPeriods = () => {
    if (selectedFrequency === "Quarterly") {
      return getMonthlyPeriodsFromQuotaPeriods();
    } else {
      // For monthly, use the periods as-is
  ...
  // Get display fields - show all periods with proper names
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white rounded-2xl w-[95%] max-w-7xl max-h-[90vh] overflow-hidden relative shadow-2xl border-0">
        ...
        {/* Quarter Grouping for Quarterly Frequency */}
        {selectedFrequency === "Quarterly" && (
          <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div className="flex flex-wrap gap-2">
              {quotaPeriods.map((quarter, index) => (
                <div key={index} className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium">
                  {quarter.period}
                </div>
              ))}
            </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-bold text-gray-900 text-base">{field.label}</h3>
                    </div>
                  ...
                  {field.dates && (
                    <p className="text-xs text-gray-500 font-medium">{field.dates}</p>
                  )}
                </div>
...
function Van_RebateSetup() {
  ...
  // Customer and Item states - UPDATED for incremental ranges with quarterly support
  const [customers, setCustomers] = useState([{ 
    code: "", 
    name: "", 
    quotas: [],
    ranges: {}, // Changed to object to support multiple periods
    qtrRebate: "" // NEW: QTR Rebate field for both fixed and incremental
  }]);
  const [items, setItems] = useState([{ 
    ...
    unitPerQty: "", 
    rebatePerBag: "",
    ranges: {} // Added ranges for products
  }]);
  ...
  useEffect(() => {
    ...
    const currentDatabase = storedUser.database || 'VAN'; // Get database from user context
    ...
    // Fetch dropdown data with current database
    fetchSalesEmployees(currentDatabase);
    fetchCustomers(currentDatabase);
    fetchItems(currentDatabase);
  }, []);
  ...
  const calculateQuotaPeriods = () => {
    ...
    if (selectedFrequency === "Monthly") {
      ...
      while (currentDate <= endDate) {
        ...
        periods.push({
          period: `Month ${periodNumber}`,
          label: `Month ${periodNumber}`,
      ...
      // Adjust to quarter start (Jan, Apr, Jul, Oct)
      const startMonth = quarterStart.getMonth();
      if (startMonth < 3) {
        quarterStart.setMonth(0); // January
      } else if (startMonth < 6) {
        quarterStart.setMonth(3); // April
      } else if (startMonth < 9) {
        quarterStart.setMonth(6); // July
      } else {
    ...
    setCustomers(prevCustomers => 
      prevCustomers.map(customer => ({
        ...
        ranges: customer.ranges || {} // Ensure ranges object exists
      }))
    );

    // Update existing items with ranges structure
    setItems(prevItems => 
      prevItems.map(item => ({
        ...item,
        ranges: item.ranges || {} // Ensure ranges object exists
      }))
    );
  };

  // UPDATED Fetch functions to use local database
  const fetchSalesEmployees = async (database = 'VAN') => {
    try {
      setLoading(true);
      const res = await fetch(`http://192.168.100.193:5000/api/sync/local/sales-employees`);
  ...
  const fetchCustomers = async (database = 'VAN') => {
    try {
      setLoading(true);
      const res = await fetch(`http://192.168.100.193:5000/api/sync/local/customers`);
  ...
  const fetchItems = async (database = 'VAN') => {
    try {
      setLoading(true);
      const res = await fetch(`http://192.168.100.193:5000/api/sync/local/items`);
  ...
  const handleRefreshData = async () => {
    try {
      ...
      // Get current database from user context or default to VAN
      const user = JSON.parse(localStorage.getItem("user")) || {};
      const currentDatabase = user.database || 'VAN';
      
      console.log(`🔄 Refreshing data from ${currentDatabase}...`);
      
      const response = await fetch('http://192.168.100.193:5000/api/sync/refresh-data', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        // Re-fetch the dropdown data after refresh
        await fetchSalesEmployees(currentDatabase);
        await fetchCustomers(currentDatabase);
        await fetchItems(currentDatabase);
        
        showToast(`Data refreshed successfully! ${currentDatabase} database updated.`, "success");
        console.log('Refresh results:', result.results);
      } else {
      ...
      dataRows.forEach((row, rowIndex) => {
        try {
          ...
          const customer = {
            ...
            ranges: {}, // Initialize empty ranges object for imported customers
            qtrRebate: "" // Initialize QTR Rebate
          };
      ...
      dataRows.forEach((row, rowIndex) => {
        try {
          ...
          const item = {
            ...
            ranges: {} // Initialize empty ranges object for imported items
          };
      ...
      row.forEach((cell, index) => {
        if (cell && typeof cell === 'string') {
          ...
          if (dataValue) {
            ...
            } else if (headerValue.includes('rebate') && headerValue.includes('type')) {
              setRebateType(dataValue.toString());
      ...
      if (type === 'customer') {
        ...
        const headerSection = [
          ...
          [
            "CustomerCode",
            "CustomerName",
            "QTR Rebate", // NEW: QTR Rebate column
            ...(quotaType === "withQuota" ? quotaPeriods.map((period, idx) => `${period.period} Quota`) : [])
          ]
        ];
        ...
        const customerData = customers
          ...
          .map(customer => [
            customer.code || '',
            customer.name || '',
            customer.qtrRebate || '', // NEW: QTR Rebate value
        ...
        const columnWidths = [
          ...
          { wch: 5 }, // spacer
          { wch: 15 }, { wch: 25 }, { wch: 12 }, // CustomerCode, CustomerName, QTR Rebate
          ...(quotaType === "withQuota" ? quotaPeriods.map(() => ({ wch: 15 })) : [])
        ];
  ...
  // ENHANCED Modal functions for both Fixed and Incremental
  ...
  // NEW: Open product range modal for incremental rebates
  const openProductRangeModal = (productIndex) => {
    if (quotaType === "withoutQuota") {
      showToast("Range management is disabled for 'Without Quota' programs", "info");
      return;
    }
    ...
    if (rebateType === "Incremental") {
      setProductRangeModal({
        isOpen: true,
        product: { ...product, index: productIndex }
      });
    } else {
      // For fixed rebate, show direct input field (no modal)
  ...
  // NEW: Handle save product ranges
  ...
  const closeRangeModal = () => {
    setRangeModal({ isOpen: false, customer: null });
  };

  // NEW: Close product range modal
  const closeProductRangeModal = () => {
    setProductRangeModal({ isOpen: false, product: null });
  };
  ...
  const handleQuotaTypeChange = (type) => {
    ...
    if (type === "withoutQuota") {
      ...
      setItems(prev => prev.map(item => ({ ...item, ranges: {} })));
      setQuotaPeriods([]);
      setQuotaCount(0);
  ...
  const handleAddCustomer = () => {
    const newCustomers = [...customers, { 
      ...
      ranges: {},
      qtrRebate: "" // NEW: Initialize QTR Rebate
    }];
    setCustomers(newCustomers);
  ...
  // NEW: Handle QTR Rebate change
  const handleQtrRebateChange = (index, value) => {
    const newData = [...customers];
    // Allow numbers, decimals, and empty values
    if (value === "" || /^-?\d*\.?\d*$/.test(value)) {
      newData[index].qtrRebate = value;
      setCustomers(newData);
    }
  };

  // NEW: Handle Rebate Per Bag change
  const handleRebatePerBagChange = (index, value) => {
    const newData = [...items];
    // Allow numbers, decimals, and empty values
    if (value === "" || /^-?\d*\.?\d*$/.test(value)) {
      newData[index].rebatePerBag = value;
      setItems(newData);
    }
  };
  ...
  const handleSave = async () => {
    // Validation
    if (!rebateType) {
      showToast("Please select a rebate type", "error");
      return;
    }
    ...
    // Additional validation for With Quota mode
    ...
    // Additional validation for incremental rebates
    if (rebateType === "Incremental" && quotaType === "withQuota") {
      const customersWithInvalidRanges = customers.filter(c => {
        // Check if ranges exist for all periods
        for (let i = 0; i < quotaPeriods.length; i++) {
          if (!c.ranges || !c.ranges[i] || c.ranges[i].length === 0 || 
              c.ranges[i].some(range => !range.min || !range.max || !range.rebate)) {
            return true;
          }
        }
        return false;
      ...
      if (customersWithInvalidRanges.length > 0) {
        showToast("Please configure rebate ranges for all periods in incremental mode", "error");
        return;
      }

      const itemsWithInvalidRanges = items.filter(i => {
        // Check if ranges exist for all periods
        for (let i = 0; i < quotaPeriods.length; i++) {
          if (!i.ranges || !i.ranges[i] || i.ranges[i].length === 0 || 
              i.ranges[i].some(range => !range.min || !range.max || !range.rebate)) {
            return true;
          }
        }
        return false;
      ...
      if (itemsWithInvalidRanges.length > 0) {
        showToast("Please configure product rebate ranges for all periods in incremental mode", "error");
        return;
      }
    }
  ...
  const resetForm = () => {
    ...
    setCustomers([{ 
      ...
      quotas: [],
      ranges: {},
      qtrRebate: "" // NEW: Reset QTR Rebate
    }]);
    setItems([{ 
      code: "", 
      name: "", 
  ...
  // UPDATED: Calculate summary for quota/ranges column
  const getQuotaSummary = (customer) => {
    if (quotaType === "withoutQuota") {
      return "No Quota";
    }
    
    if (rebateType === "Incremental") {
      const totalRanges = Object.values(customer.ranges || {}).reduce((total, ranges) => total + (ranges?.length || 0), 0);
      return `${quotaPeriods.length} periods, ${totalRanges} ranges`;
    } else {
      return `${quotaPeriods.length} periods`;
    }
  };

  // NEW: Calculate summary for product ranges column
  const getProductRangeSummary = (product) => {
    ...
    if (rebateType === "Incremental") {
      const totalRanges = Object.values(product.ranges || {}).reduce((total, ranges) => total + (ranges?.length || 0), 0);
      return `${quotaPeriods.length} periods, ${totalRanges} ranges`;
    } else {
      return product.rebatePerBag ? `₱${product.rebatePerBag}/bag` : "Not set"; // CHANGED: Dollar to Peso sign
    }
  };
  ...
  return (
    <div className="flex min-h-screen w-full bg-gradient-to-br from-slate-50 to-blue-50 font-poppins text-slate-900">
      ...
      >
        ...
        <div className="pt-16 flex-1 p-8">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-white/50 shadow-2xl p-8 w-full max-w-[1600px] mx-auto mt-6">
            ...
            <div className="flex items-center justify-between mb-8 pb-6 border-b border-blue-200">
            ...
            <div className="mb-8 pb-8 border-b border-blue-200">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div 
                  ...
                  onClick={() => setRebateType("Fixed")}
                >
                  <div className="flex items-center gap-4 mb-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                      rebateType === "Fixed" ? "bg-blue-500" : "bg-gray-100"
                    }`}>
                      <LocateFixed className={`w-6 h-6 ${rebateType === "Fixed" ? "text-white" : "text-gray-500"}`} />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Fixed Rebate</h3>
                      <p className="text-sm text-gray-600">Standard fixed rebate amounts per period</p>
                    </div>
                  </div>
                  <ul className="text-xs text-gray-600 space-y-2">
                    <li className="flex items-start gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                        rebateType === "Fixed" ? "bg-blue-500" : "bg-gray-400"
                      }`}></div>
                      <span>Fixed rebate amounts for each period</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                        rebateType === "Fixed" ? "bg-blue-500" : "bg-gray-400"
                      }`}></div>
                      <span>Consistent targets throughout the program</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                        rebateType === "Fixed" ? "bg-blue-500" : "bg-gray-400"
                      }`}></div>
                      <span>Simple and predictable rebate structure</span>
                    </li>
                  </ul>
                </div>

                {/* Incremental Rebate Option */}
                <div 
                  className={`border-2 rounded-2xl p-6 cursor-pointer transition-all duration-300 hover:shadow-lg ${
                  ...
                  onClick={() => setRebateType("Incremental")}
                >
                  <div className="flex items-center gap-4 mb-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                      rebateType === "Incremental" ? "bg-green-500" : "bg-gray-100"
                    }`}>
                      <TrendingUp className={`w-6 h-6 ${rebateType === "Incremental" ? "text-white" : "text-gray-500"}`} />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-gray-800">Incremental Rebate</h3>
                      <p className="text-sm text-gray-600">Progressive rebates with quarterly ranges</p>
                    </div>
                  </div>
                  <ul className="text-xs text-gray-600 space-y-2">
                    <li className="flex items-start gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                        rebateType === "Incremental" ? "bg-green-500" : "bg-gray-400"
                      }`}></div>
                      <span>Different ranges for each quarter/period</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                        rebateType === "Incremental" ? "bg-green-500" : "bg-gray-400"
                      }`}></div>
                      <span>Copy ranges between periods easily</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                        rebateType === "Incremental" ? "bg-green-500" : "bg-gray-400"
                      }`}></div>
                      <span>Flexible structure for seasonal variations</span>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            {/* Header Section with Requested Details - Only show if rebate type is selected */}
            {rebateType && (
              <>
                ...
                <div className="flex items-center justify-between mb-8 border-b border-gray-200">
                  <div className="flex">
                    ...
                    >
                      ...
                      {rebateType === "Incremental" ? "Customer Ranges" : "Customer Quotas"}
                    </button>
                    <button 
                      className={`px-8 py-4 font-semibold text-sm border-b-2 transition-all flex items-center gap-3 ${
                        activeTab === "Items" 
                    ...
                    >
                      ...
                      {rebateType === "Incremental" ? "Product Ranges" : "Product Rebates"}
                    </button>
                  </div>
                ...
                {/* Enhanced Customer Table with Dropdowns */}
                {activeTab === "Customer" && (
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-xl font-bold text-gray-800">
                        {rebateType === "Incremental" ? "Customer Rebate Ranges" : "Customer Quota Setup"}
                      </h3>
                      <div className="flex items-center gap-4">
                        ...
                        <table className="w-full border-collapse text-sm text-gray-700 min-w-[800px]">
                          <thead className="bg-gradient-to-r from-gray-50 to-blue-50 sticky top-0 z-10">
                            <tr>
                              ...
                              <th className="px-6 py-4 text-center font-bold text-gray-700 text-xs uppercase tracking-wider border-b border-gray-200 w-40">
                                {quotaType === "withQuota" ? (rebateType === "Incremental" ? "Rebate Ranges" : "Quotas") : "Status"}
                              </th>
                                ...
                                <td className="px-6 py-4">
                                  {isRowEditable('customer', idx) ? (
                                    <input
                                      type="text"
                                      value={c.qtrRebate || ""}
                                      onChange={(e) => handleQtrRebateChange(idx, e.target.value)}
                                      placeholder="Enter QTR Rebate"
                                ...
                                <td className="px-6 py-4">
                                  {quotaType === "withQuota" ? (
                                    <button
                                      ...
                                      className={`w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg transition-all shadow-sm text-sm font-medium ${
                                        rebateType === "Incremental" 
                ...
                {/* Enhanced Items Table with Range Setup */}
                {activeTab === "Items" && (
                  <div className="mb-8">
                    <h3 className="text-xl font-bold text-gray-800 mb-6">
                      {rebateType === "Incremental" ? "Product Rebate Ranges" : "Product Rebate Configuration"}
                    </h3>
                    <div className="border border-gray-200 rounded-2xl overflow-hidden max-h-96 overflow-y-auto bg-white shadow-sm">
                      <div className="overflow-x-auto">
                        <table className="w-full border-collapse text-sm text-gray-700 min-w-[600px]">
                          <thead className="bg-gradient-to-r from-gray-50 to-purple-50 sticky top=0 z-10">
                            <tr>
                              ...
                              <th className="px-6 py-4 text-center font-bold text-gray-700 text-xs uppercase tracking-wider border-b border-gray-200 w-40">
                                {rebateType === "Incremental" ? "Rebate Ranges" : "Rebate Per Bag"}
                              </th>
                              <th className="px-6 py-4 text-center font-bold text-gray-700 text-xs uppercase tracking-wider border-b border-gray-200 w-28">
                                Actions
                              </th>
                          ...
                          <tbody>
                            {items.map((i, idx) => (
                              <tr key={idx} className="even:bg-gray-50 hover:bg-purple-50 transition-colors group border-b border-gray-100 last:border-b-0">
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                  {rebateType === "Incremental" ? (
                                    <button
                                      onClick={() => openProductRangeModal(idx)}
                                      className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-gradient-to-r from-purple-500 to-purple-600 text-white hover:from-purple-600 hover:to-purple-700 rounded-lg transition-all shadow-sm text-sm font-medium"
                                    >
                                      <TrendingUp className="w-4 h-4" />
                                      {getProductRangeSummary(i)}
                                    </button>
                                  ) : (
                                    <div className="w-full">
                                      {isRowEditable('item', idx) ? (
                                        ...
                                        <div className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-lg text-sm font-medium">
                                          <span className="text-xs">₱</span> {/* CHANGED: Dollar to Peso sign */}
                                          {getProductRangeSummary(i).replace('$', '₱')} {/* CHANGED: Dollar to Peso sign */}
                                        </div>
                                      )}
                                    </div>
                                  )}
      ...
      {/* Incremental Rebate Range Modal */}
      <RangeModal
        isOpen={rangeModal.isOpen}
        onClose={closeRangeModal}
        customer={rangeModal.customer}
        onSave={(ranges) => handleSaveRanges(rangeModal.customer.index, ranges)}
        quotaPeriods={quotaPeriods}
        rebateType={rebateType}
        quotaType={quotaType}
      />
      ...
      {/* Product Range Modal */}
      <ProductRangeModal
        isOpen={productRangeModal.isOpen}
        onClose={closeProductRangeModal}
        product={productRangeModal.product}
        onSave={(ranges) => handleSaveProductRanges(productRangeModal.product.index, ranges)}
        quotaPeriods={quotaPeriods}
        rebateType={rebateType}
        quotaType={quotaType}
      />
