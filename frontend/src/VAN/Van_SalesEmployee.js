...
function Van_SalesEmployee() {
  ...
  // ✅ Fetch Sales Employees from local VAN_OWN database
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const res = await fetch("http://192.168.100.193:3006/api/sync/local/sales-employees?db=VAN_OWN");
        ...
        console.log(`Loaded ${employeesData.length} sales employees from VAN_OWN database`);
      } catch (err) {
            ...
            {/* Modern Header */}
              <div className={`flex items-center gap-3 mb-6 pb-4 border-b ${
                ...
                <div>
                  ...
                  }`}>Manage and view all sales employee information from VAN_OWN database</p>
                </div>
              </div>
