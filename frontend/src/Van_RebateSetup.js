...
const RangeModal = ({ isOpen, onClose, customer, onSave, quotaPeriods, importedQuotas = [], rebateType = "Fixed" }) => {
...
const QuotaModal = ({ isOpen, onClose, customer, onSave, quotaPeriods, importedQuotas = [], rebateType = "Fixed" }) => {
...
function Van_RebateSetup() {
  ...
  // Enhanced quota calculation based on date range and frequency
  useEffect(() => {
    if (selectedDateFrom && selectedDateTo && selectedFrequency) {
      ...
      setQuotaPeriods([]);
      setQuotaCount(0);
      // Reset customer quotas when no periods
      setCustomers(prev => prev.map(c => ({ ...c, quotas: [] })));
    }
  }, [selectedDateFrom, selectedDateTo, selectedFrequency]);
  ...
  const calculateQuotaPeriods = () => {
    ...
    setCustomers(prevCustomers => 
      prevCustomers.map(customer => ({
        ...customer,
        quotas: Array(periods.length).fill(""),
  ...
  const fetchSalesEmployees = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://192.168.100.193:5000/api/sales-employees");
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://192.168.100.193:5000/api/items");
  ...
  // ENHANCED Export function with proper quota alignment
      ...
      if (type === 'customer') {
        ...
        const headerSection = [
          ...
          [
            "CustomerCode",
            "CustomerName",
            ...quotaPeriods.map((period, idx) => `${period.period} Quota`)
          ]
        ];
        ...
        const customerData = customers
          ...
          .map(customer => [
            customer.code || '',
            customer.name || '',
            ...customer.quotas.map(quota => quota || '')
          ]);
        ...
        showToast("Customer data exported successfully with aligned quotas!", "success");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast("Rebate setup saved successfully!", "success");
      ...
      >
        ...
        <div className="pt-16 flex-1 p-8">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-white/50 shadow-2xl p-8 w-full max-w-[1600px] mx-auto mt-6">
                ...
                >
                  <div className="flex items-center gap-4 mb-4">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                      ...
                      <DollarSign className={`w-6 h-6 ${rebateType === "Fixed" ? "text-white" : "text-gray-500"}`} />
                    </div>
                    <div>
            ...
            {rebateType && (
              <>
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8 pb-8 border-b border-blue-200">
                  <div className="space-y-3">
                  ...
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="text-sm text-gray-500 bg-blue-50 px-3 py-1 rounded-full">
                        {customers.length} customer(s) • {quotaPeriods.length} period{quotaPeriods.length !== 1 ? 's' : ''}
                      </div>
                    </div>
                          ...
                          <thead className="bg-gradient-to-r from-gray-50 to-blue-50 sticky top-0 z-10">
                            <tr>
                              ...
                              <th className="px-6 py-4 text-center font-bold text-gray-700 text-xs uppercase tracking-wider border-b border-gray-200 w-40">
                                {rebateType === "Incremental" ? "Rebate Ranges" : "Quotas"}
                              </th>
