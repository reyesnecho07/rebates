...
const QuotaModal = ({ isOpen, onClose, customer, onSave, quotaPeriods, importedQuotas = [] }) => {
...
function RebateSetup() {
      ...
      if (type === 'customer') {
        ...
        const headerSection = [
          ...
          [
            "CustomerCode",
            "CustomerName",
            ...quotaPeriods.map((_, idx) => `Quota${idx + 1}`)
          ]
        ];
  ...
  // ENHANCED Quota modal functions with imported quotas
  ...
  const handleAddCustomer = () => {
    const newCustomers = [...customers, { code: "", name: "", quotas: Array(quotaCount).fill("") }];
    setCustomers(newCustomers);
  ...
  const resetForm = () => {
    setCustomers([{ code: "", name: "", quotas: Array(quotaCount).fill("") }]);
  ...
  // UPDATED: Calculate summary for quota column - Show period count instead of filled quotas
      ...
      >
        ...
        <div className="pt-16 flex-1 p-8">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-white/50 shadow-2xl p-8 w-full max-w-[1600px] mx-auto mt-6">
            ...
            {/* Header Section with Requested Details */}
          ...
          {/* Enhanced Mechanics Card */}
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-5 w-full max-w-[1600px] mx-auto mt-6 shadow-lg">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center">
                <Info className="w-4 h-4 text-white" />
              </div>
              <div>
                <h3 className="text-base font-bold text-amber-800">Program Mechanics & Guidelines</h3>
                <p className="text-xs text-amber-700">Important terms and conditions for the rebate program</p>
              </div>
            </div>
            <ul className="text-xs text-amber-800 space-y-1.5">
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1.5 flex-shrink-0"></div>
                <span><strong>Kitanex Partner Clerk</strong> is entitled to the above-mentioned rate per bottle.</span>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1.5 flex-shrink-0"></div>
                <span><strong>Monthly Incentive Release:</strong> Kitanex incentive will be released to Dealers monthly upon reorder in the form of a check on or before the 15th day of the succeeding month.</span>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1.5 flex-shrink-0"></div>
                <span><strong>Dealer Responsibility:</strong> Kitanex Dealer must release the incentive to our Kitanex Partner Clerk.</span>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1.5 flex-shrink-0"></div>
                <span><strong>Documentation Requirement:</strong> Kitanex Partner Clerk must present his/her record to the store owner for releasing of KITANEX.</span>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-1.5 flex-shrink-0"></div>
                <span><strong>Management Rights:</strong> The management reserves the right to change or modify any of the terms and conditions contained in this program once incentive for clerk is not released.</span>
              </li>
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-red-500 mt-1.5 flex-shrink-0"></div>
                <span className="font-bold text-red-700">STRICTLY NO RETURN POLICY.</span>
              </li>
            </ul>
          </div>
        </div>
