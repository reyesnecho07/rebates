// VcpTransactionRecords.jsx
import React, { useEffect } from 'react';
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, FileText, ClipboardList } from 'lucide-react';
...
const VcpTransactionRecords = ({
  ...
  // ── Helpers ───────────────────────────────────────────────────────────────
  const isInvoice = (t) => {
    const tt = t.Treetype || t.treetype || t.type;
    return tt === 'I';
  };
        ...
        {paginated.length > 0 ? (
          <table className="w-full text-xs table-fixed">
            <thead className={`sticky top-0 border-b ${T.thead}`}>
              <tr>
                <th className={`px-4 pl-6 py-2.5 text-left text-[10px] font-bold uppercase tracking-widest whitespace-nowrap w-[80px] ${T.ts}`}>
                  Date
                </th>
                <th className={`px-4 py-2.5 text-left text-[10px] font-bold uppercase tracking-widest w-[300px] ${T.ts}`}>
                  Item
                </th>
                <th className={`px-4 py-2.5 text-left text-[10px] font-bold uppercase tracking-widest w-[80px] ${T.ts}`}>
                  Qty
                </th>
                <th className={`px-4 py-2.5 text-left text-[10px] font-bold uppercase tracking-widest w-[80px] ${T.ts}`}>
                  P. Value
            ...
            <tbody className={`divide-y ${T.divider}`}>
              {paginated.map((t, i) => {
                ...
                const is25kg      = t.Is25kgItem || itemName.toLowerCase().includes('25kg');
                const displayName = is25kg ? `${itemName} (25kg)` : itemName;
                return (
                  <tr
                    key={i}
                    className={`transition-colors duration-100 border-b ${T.row} ${
                      inv ? 'opacity-60 italic' : ''
                    }`}
                  >
                    {/* Date */}
                    <td className="px-4 pl-6 py-2.5 w-[120px] text-left">
                      <span className={`font-medium tabular-nums whitespace-nowrap ${T.tp}`}>
                        {fmtDate(t.Date)}
                      </span>
                    </td>
                    {/* Item */}
                    <td className="px-4 py-2.5 w-[160px] text-left">
                      <div
                        className={`font-medium ${T.tp}`}
                        style={{
                          display: '-webkit-box',
                          WebkitLineClamp: 2,
                    ...
                    {/* Qty */}
                    <td className="px-4 py-2.5 text-left w-[70px]">
                    ...
                    {/* P. Value */}
                    <td className="px-4 py-2.5 text-left w-[100px]">
                      <span className={`inline-block px-2 py-0.5 rounded border font-semibold tabular-nums ${
                        isDark
