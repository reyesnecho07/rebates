...
IF EXISTS (SELECT 1 FROM ORIN WHERE DocNum = (SELECT DocNum FROM [192.168.11.103].ZZZ_VAN_TEST.dbo.ORIN WHERE DocEntry = @list_of_cols_val_tab_del))
	BEGIN
		UPDATE
			ORIN
		SET
		...
		FROM
			[192.168.11.103].ZZZ_VAN_TEST.dbo.ORIN T0
		WHERE
...
ELSE
	BEGIN
		...
		FROM
			[192.168.11.103].ZZZ_VAN_TEST.dbo.ORIN T0
			INNER JOIN [192.168.11.103].ZZZ_VAN_TEST.dbo.RIN1 T1 ON T0.DocEntry = T1.DocEntry
			INNER JOIN [192.168.11.103].ZZZ_VAN_TEST.dbo.OITM T2 ON T1.ItemCode = T2.ItemCode
			INNER JOIN [192.168.11.103].ZZZ_VAN_TEST.dbo.OITB T3 ON T2.ItmsGrpCod = T3.ItmsGrpCod AND T3.ItmsGrpNam IN ('FEEDTAG', 'VETERINARY', 'REBATES')
		WHERE
			T0.DocEntry = @list_of_cols_val_tab_del
