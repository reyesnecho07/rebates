...
SELECT
	...
	T1.LineMemo
FROM
	[192.168.11.103].ZZZ_VAN_TEST.dbo.OJDT T0
	INNER JOIN [192.168.11.103].ZZZ_VAN_TEST.dbo.JDT1 T1 ON T0.TransId = T1.TransId
	INNER JOIN [192.168.11.103].ZZZ_VAN_TEST.dbo.JDT1 T2 ON T0.TransId = T2.TransId AND T2.ShortName IN (SELECT CardCode FROM [192.168.11.103].ZZZ_VAN_TEST.dbo.OCRD WHERE CardType = 'C')
	LEFT JOIN [192.168.11.103].ZZZ_VAN_TEST.dbo.OCRD T4 ON T2.ShortName = T4.CardCode
	LEFT JOIN [192.168.11.103].ZZZ_VAN_TEST.dbo.OACT T3 ON T1.Account = T3.AcctCode
WHERE
