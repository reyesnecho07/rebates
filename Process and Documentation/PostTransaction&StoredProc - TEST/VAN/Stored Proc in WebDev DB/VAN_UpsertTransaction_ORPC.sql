...
IF EXISTS (SELECT 1 FROM ORPC WHERE DocNum = (SELECT DocNum FROM [192.168.11.103].ZZZ_VAN_TEST.dbo.ORPC WHERE DocEntry = @list_of_cols_val_tab_del))
	BEGIN
		UPDATE
			ORPC
		SET
		...
		FROM
			[192.168.11.103].ZZZ_VAN_TEST.dbo.ORPC T0
		WHERE
...
ELSE
	BEGIN
		...
		FROM
			[192.168.11.103].ZZZ_VAN_TEST.dbo.ORPC T0
			INNER JOIN [192.168.11.103].ZZZ_VAN_TEST.dbo.RPC1 T1 ON T0.DocEntry = T1.DocEntry
		WHERE
			T0.DocEntry = @list_of_cols_val_tab_del
