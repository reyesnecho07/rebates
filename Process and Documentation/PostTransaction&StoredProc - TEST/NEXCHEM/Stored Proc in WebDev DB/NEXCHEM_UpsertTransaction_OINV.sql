...
IF EXISTS (SELECT 1 FROM OINV WHERE DocNum = (SELECT DocNum FROM [192.168.11.103].ZZZ_NEXCHEM_TEST.dbo.OINV WHERE DocEntry = @list_of_cols_val_tab_del))
	BEGIN
		UPDATE
			OINV
		SET
		...
		FROM
			[192.168.11.103].ZZZ_NEXCHEM_TEST.dbo.OINV T0
		WHERE
...
ELSE
	BEGIN
		...
		FROM
			[192.168.11.103].ZZZ_NEXCHEM_TEST.dbo.OINV T0
			INNER JOIN [192.168.11.103].ZZZ_NEXCHEM_TEST.dbo.INV1 T1 ON T0.DocEntry = T1.DocEntry
