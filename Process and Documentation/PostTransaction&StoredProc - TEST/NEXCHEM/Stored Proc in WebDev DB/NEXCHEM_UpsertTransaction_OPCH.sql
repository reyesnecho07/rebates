...
IF EXISTS (SELECT 1 FROM OPCH WHERE DocNum = (SELECT DocNum FROM [192.168.11.103].ZZZ_NEXCHEM_TEST.dbo.OPCH WHERE DocEntry = @list_of_cols_val_tab_del))
	BEGIN
		UPDATE
			OPCH
		SET
		...
		FROM
			[192.168.11.103].ZZZ_NEXCHEM_TEST.dbo.OPCH T0
		WHERE
...
ELSE
	BEGIN
		...
		FROM
			[192.168.11.103].ZZZ_NEXCHEM_TEST.dbo.OPCH T0
			INNER JOIN [192.168.11.103].ZZZ_NEXCHEM_TEST.dbo.PCH1 T1 ON T0.DocEntry = T1.DocEntry
		WHERE
			T0.DocEntry = @list_of_cols_val_tab_del
