...
IF EXISTS (SELECT 1 FROM OINV WHERE DocNum = (SELECT DocNum FROM [192.168.11.103].ZZZ_VCP_TEST.OINV WHERE DocEntry = @list_of_cols_val_tab_del))
	BEGIN
		UPDATE
			OINV
		SET
		...
		FROM
			[192.168.11.103].ZZZ_VCP_TEST.OINV T0
		WHERE
...
ELSE
	BEGIN
		...
		FROM
			[192.168.11.103].ZZZ_VCP_TEST.OINV T0
			INNER JOIN [192.168.11.103].ZZZ_VCP_TEST.INV1 T1 ON T0.DocEntry = T1.DocEntry
			INNER JOIN [192.168.11.103].ZZZ_VCP_TEST.OITM T2 ON T1.ItemCode = T2.ItemCode
			INNER JOIN [192.168.11.103].ZZZ_VCP_TEST.OITB T3 ON T2.ItmsGrpCod = T3.ItmsGrpCod AND T3.ItmsGrpNam IN ('CHEMICALS', 'FERTILIZERS', 'SPECIAL PRODUCT', 'REBATES')
		WHERE
			T0.DocEntry = @list_of_cols_val_tab_del
