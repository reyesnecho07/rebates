...
FROM
    (
		SELECT
			...
			T0.ItmsGrpCod
		FROM
			[192.168.11.103].ZZZ_VAN_TEST.dbo.OITM T0
		WHERE
			(
				NOT
				(
				...
				(
					SELECT DISTINCT
						A1.ItemCode
					FROM
						ZZZ_VAN_DB.dbo.FixProdRebate A1
					...
					SELECT DISTINCT
						A2.ItemCode
					FROM
						ZZZ_VAN_DB.dbo.IncItemRebate A2
					...
					SELECT DISTINCT
						A3.ItemCode
					FROM
						ZZZ_VAN_DB.dbo.PerProdRebate A3
				)
			)
    ) T0
