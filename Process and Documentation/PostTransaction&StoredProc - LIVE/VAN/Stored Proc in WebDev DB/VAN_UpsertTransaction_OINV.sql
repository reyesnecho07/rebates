...
IF EXISTS (SELECT 1 FROM OINV WHERE DocNum = (SELECT DocNum FROM [192.168.11.103].VAN.dbo.OINV WHERE DocEntry = @list_of_cols_val_tab_del))
	BEGIN
		UPDATE
			OINV
		SET
		...
		FROM
			[192.168.11.103].VAN.dbo.OINV T0
		WHERE
...
ELSE
	BEGIN
		...
		FROM
			[192.168.11.103].VAN.dbo.OINV T0
			INNER JOIN [192.168.11.103].VAN.dbo.INV1 T1 ON T0.DocEntry = T1.DocEntry
			INNER JOIN [192.168.11.103].VAN.dbo.OITM T2 ON T1.ItemCode = T2.ItemCode
			INNER JOIN [192.168.11.103].VAN.dbo.OITB T3 ON T2.ItmsGrpCod = T3.ItmsGrpCod AND T3.ItmsGrpNam IN ('FEEDTAG', 'VETERINARY', 'REBATES')
		WHERE
			T0.DocEntry = @list_of_cols_val_tab_del
