...
IF EXISTS (SELECT 1 FROM ORIN WHERE DocNum = (SELECT DocNum FROM [192.168.11.103].ZZZ_VCP_TEST.ORIN WHERE DocEntry = @list_of_cols_val_tab_del))
	BEGIN
		UPDATE
			ORIN
		SET
		...
		FROM
			[192.168.11.103].ZZZ_VCP_TEST.ORIN T0
		WHERE
...
ELSE
	BEGIN
		...
		FROM
			[192.168.11.103].ZZZ_VCP_TEST.ORIN T0
			INNER JOIN [192.168.11.103].ZZZ_VCP_TEST.RIN1 T1 ON T0.DocEntry = T1.DocEntry
